...
static NSString *appctrl_webview_user_script_source(void) {
    ...
    return [NSString stringWithFormat:
            @"(function(){\n"
             ...
             "};\n"
             // 在白名单模式下，监听动态插入的图片/媒体，阻止非白名单域名的资源加载
             "if(white.length>0){\n"
             "  __log('白名单模式激活,域名数:'+white.length);\n"
             ...
             "        if((tag==='img'||tag==='iframe'||tag==='video'||tag==='audio')&&node.src){\n"
             "          __log('检测到媒体元素:'+tag+' src='+node.src.substr(0,50));\n"
             "          if(shouldBlock(node.src)){\n"
             "            __log('阻止加载:'+node.src.substr(0,50));\n"
             "            node.src='';\n"
             "            node.removeAttribute('src');\n"
             "            __hideAdContainer(node);\n"
             "          }\n"
             "        }\n"
             "        // 递归检查子元素\n"
             "        if(node.querySelectorAll){\n"
             "          var imgs=node.querySelectorAll('img[src],iframe[src],video[src],audio[src]');\n"
             "          if(imgs.length>0){__log('发现'+imgs.length+'个子媒体元素');}\n"
             "          for(var j=0;j<imgs.length;j++){\n"
             "            __log('检查子元素:'+imgs[j].tagName+' src='+imgs[j].src.substr(0,50));\n"
             "            if(shouldBlock(imgs[j].src)){\n"
             "              __log('阻止子元素:'+imgs[j].src.substr(0,50));\n"
             "              imgs[j].src='';\n"
             "              imgs[j].removeAttribute('src');\n"
             "              __hideAdContainer(imgs[j]);\n"
             "            }\n"
             "          }\n"
             "        }\n"
             "      }\n"
             ...
             "    __log('MutationObserver已启动');\n"
             "  }\n"
             "  // 扫描已存在的图片\n"
             "  function scanExistingImages(){\n"
             "    try{\n"
             "      var all=document.querySelectorAll('img[src],iframe[src],video[src],audio[src]');\n"
             "      __log('扫描已有媒体元素:'+all.length+'个');\n"
             "      for(var i=0;i<all.length;i++){\n"
             "        if(shouldBlock(all[i].src)){\n"
             "          __log('移除已有元素:'+all[i].src.substr(0,50));\n"
             "          all[i].src='';\n"
             ...
             "    +'ins.adsbygoogle,ins.adsbygoogle-noablate,[class*=\"adsbygoogle\"],ins[class*=\"ad-\"],'\n"
             "    +'iframe[src*=\"googlesyndication\"],iframe[src*=\"doubleclick\"],iframe[id^=\"aswift_\"],iframe[id^=\"google_ads_iframe\"],'\n"
             "    +'div[id*=\"google_ads\"],div[class*=\"google-ads\"],div[class*=\"google-auto-placed\"],div[data-ad-client],'\n"
             ...
             "  }catch(e){}\n"
             "}\n"
             "function __looksLikeCloseButton(el){\n"
             "  try{\n"
             "    if(!el||!el.tagName){return false;}\n"
             "    var text=(el.textContent||'').replace(/\\s+/g,'').toLowerCase();\n"
             "    var cls=((typeof el.className==='string'?el.className:'')+' '+(el.id||'')+' '+(el.getAttribute('aria-label')||'')+' '+(el.getAttribute('title')||'')).toLowerCase();\n"
             "    if(text==='×'||text==='x'||text==='✕'||text==='✖'||text==='关闭'||text==='關閉'||text==='skip'){return true;}\n"
             "    return /close|关闭|關閉|dismiss|skip/.test(cls);\n"
             "  }catch(e){return false;}\n"
             "}\n"
             "function __isTinyOverlayControl(el){\n"
             "  try{\n"
             "    if(!el||!el.getBoundingClientRect){return false;}\n"
             "    var rect=el.getBoundingClientRect();\n"
             "    if(rect.width<=0||rect.height<=0||rect.width>90||rect.height>90){return false;}\n"
             "    var pos=window.getComputedStyle(el).position;\n"
             "    return pos==='absolute'||pos==='fixed';\n"
             "  }catch(e){return false;}\n"
             "}\n"
             "function __hideNearbyCloseButtons(el){\n"
             "  try{\n"
             "    var containers=[];\n"
             "    if(el){containers.push(el);}\n"
             "    if(el&&el.parentElement){containers.push(el.parentElement);}\n"
             "    if(el&&el.parentElement&&el.parentElement.parentElement){containers.push(el.parentElement.parentElement);}\n"
             "    for(var c=0;c<containers.length;c++){\n"
             "      var root=containers[c];\n"
             "      if(!root||!root.querySelectorAll){continue;}\n"
             "      var nodes=root.querySelectorAll('button,a,div,span,i');\n"
             "      for(var i=0;i<nodes.length;i++){\n"
             "        var node=nodes[i];\n"
             "        if(node.getAttribute('data-appctrl-ad')==='1'){continue;}\n"
             "        if(__looksLikeCloseButton(node)||__isTinyOverlayControl(node)){\n"
             "          __markAd(node);\n"
             "        }\n"
             "      }\n"
             "    }\n"
             "  }catch(e){}\n"
             "}\n"
             // 统一的广告扫描器：每秒扫描一次全页面，持续 10 秒，兜底覆盖所有动态注入手法
             // (无论 MutationObserver 是否捕捉到插入过程，最终 DOM 里的源码状态都会被扫到)
             // 使用分批异步执行，避免阻塞 UI 主线程
             "function __hideAdContainer(el){\n"
             "  var node=el;\n"
             ...
             "      if(tag==='ins'||cls.indexOf('adsbygoogle')!==-1||cls.indexOf('google-auto-placed')!==-1||id.indexOf('google_ads')!==-1){__markAd(node);__hideNearbyCloseButtons(node);return;}\n"
             "      var pos=window.getComputedStyle(node).position;\n"
             "      if(pos==='fixed'||pos==='sticky'){__markAd(node);__hideNearbyCloseButtons(node);return;}\n"
             "    }catch(e2){}\n"
             "    node=node.parentNode;\n"
             "  }\n"
             "  __markAd(el);\n"
             "  __hideNearbyCloseButtons(el);\n"
             "}\n"
             "var __scanCount=0;\n"
             "var __scanInterval=null;\n"
             "function __scanAllAds(){\n"
             "  __scanCount++;\n"
             "  __log('开始第'+__scanCount+'次扫描');\n"
             "  try{\n"
             "    // 1. 白名单模式下扫描非白名单图片/媒体元素，隐藏整个广告容器\n"
             "    if(white.length>0){\n"
             "      var all=document.querySelectorAll('img[src],iframe[src],video[src],audio[src]');\n"
             "      __log('扫描发现'+all.length+'个媒体元素');\n"
             "      var blockedCount=0;\n"
             "      for(var i=0;i<all.length;i++){\n"
             "        var el=all[i];\n"
             "        if(el.src&&shouldBlock(el.src)&&el.getAttribute('data-appctrl-ad')!=='1'){\n"
             "          __log('拦截非白名单元素:'+el.tagName+' '+el.src.substr(0,60));\n"
             "          el.src='';\n"
             "          el.removeAttribute('src');\n"
             "          __hideAdContainer(el);\n"
             "          blockedCount++;\n"
             "        }\n"
             "      }\n"
             "      if(blockedCount>0){__log('本轮拦截'+blockedCount+'个非白名单媒体');}\n"
             "    }\n"
             "    // 2. 扫描 base64 背景图（异步分批，避免阻塞）\n"
             "    (function(){\n"
             ...
             "        var end=Math.min(idx+batchSize,allEls.length);\n"
             "        for(;idx<end;idx++){\n"
             "          try{\n"
             "            var el=allEls[idx];\n"
             "            var computed=window.getComputedStyle(el);\n"
             "            var bg=computed.backgroundImage;\n"
             "            if(bg&&bg.indexOf('data:image')!==-1&&bg.indexOf('base64')!==-1){\n"
             "              __log('拦截base64背景图:'+el.tagName+' bg='+bg.substr(0,80));\n"
             ...
             "          __log('本轮拦截'+base64Count+'个base64背景图');\n"
             "        }\n"
             "      }\n"
             "      processBatch();\n"
             "    })();\n"
             "    // 3. 已知广告选择器（同步，数量少）\n"
             "    var ads=document.querySelectorAll('ins.adsbygoogle,ins.adsbygoogle-noablate,[class*=\"adsbygoogle\"],ins[class*=\"ad-\"],'+'iframe[src*=\"googlesyndication\"],iframe[src*=\"doubleclick\"],iframe[id^=\"aswift_\"],iframe[id^=\"google_ads_iframe\"],'+'div[id*=\"google_ads\"],div[class*=\"google-ads\"],div[class*=\"google-auto-placed\"],div[data-ad-client]');\n"
             "    if(ads.length>0){__log('发现'+ads.length+'个已知广告选择器');}\n"
             "    for(var k=0;k<ads.length;k++){__markAd(ads[k]);__hideNearbyCloseButtons(ads[k]);}\n"
             "  }catch(e){__log('扫描异常:'+e.message);}\n"
             "}\n"
             // 暴露重启扫描的接口，供 Save 按钮触发时调用
             "window.__appctrlRestartScan=function(newWhite,newBlocked){\n"
             "  if(newWhite!==undefined){white=newWhite;}\n"
             "  if(newBlocked!==undefined){blocked=newBlocked;}\n"
             "  __scanCount=0;\n"
             "  if(__scanInterval){clearInterval(__scanInterval);}\n"
             "  __scanAllAds();\n"
             "  __scanInterval=setInterval(function(){\n"
             "    __scanAllAds();\n"
             "    if(__scanCount>=10){clearInterval(__scanInterval);__scanInterval=null;}\n"
             "  },1000);\n"
             "};\n"
             "if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',function(){window.__appctrlRestartScan();});}else{window.__appctrlRestartScan();}\n"
             "})();",

...
static NSString *appctrl_webview_user_script_source(void) {
    ...
    return [NSString stringWithFormat:
            @"(function(){\n"
             ...
             "      if(tag==='ins'){__markAd(node);return;}\n"
             "      var pos=window.getComputedStyle(node).position;\n"
             "      if(pos==='fixed'||pos==='sticky'){__markAd(node);return;}\n"
             "    }catch(e2){}\n"
             "    node=node.parentNode;\n"
             "  }\n"
             "  __markAd(el);\n"
             ...
             "    })();\n"
             "    // 3. 已知广告选择器（同步，数量少）\n"
             "    var ads=document.querySelectorAll('ins.adsbygoogle,[class*=\"adsbygoogle\"],ins[class*=\"ad-\"],'+'iframe[src*=\"googlesyndication\"],iframe[src*=\"doubleclick\"],'+'div[id*=\"google_ads\"],div[class*=\"google-ads\"]');\n"
             "    if(ads.length>0){__log('发现'+ads.length+'个已知广告选择器');}\n"
             "    for(var k=0;k<ads.length;k++){__markAd(ads[k]);}\n"

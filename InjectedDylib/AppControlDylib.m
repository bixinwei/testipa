...
static NSString *appctrl_webview_user_script_source(void) {
    ...
    return [NSString stringWithFormat:
            @"(function(){\n"
             ...
             "  }catch(e){}\n"
             "}\n"
             // 只隐藏已知广告选择器（Google Ads 等明确特征），不再启发式扫描
             // fixed/sticky+含图元素，因为大量正常网站的固定导航栏/悬浮按钮也会
             // 用到图片，之前的启发式扫描会把正常图片一起隐藏掉。
             "function removeKnownAds(){\n"
             "  try{\n"
             "    var sel='ins.adsbygoogle,[class*=\"adsbygoogle\"],ins[class*=\"ad-\"],'\n"
             "      +'iframe[src*=\"googlesyndication\"],iframe[src*=\"doubleclick\"],'\n"
             "      +'div[id*=\"google_ads\"],div[class*=\"google-ads\"]';\n"
             "    var ads=document.querySelectorAll(sel);\n"
             "    for(var i=0;i<ads.length;i++){__markAd(ads[i]);}\n"
             "  }catch(e){}\n"
             "}\n"
             ...
             // 拦截通过 style background-image:url(data:image/...;base64,...) 注入的广告图片。
             // 这类广告常见手法：绕开 <img src> 的静态规则检测，直接把图片编码进 CSS 背景。
             "function __stripBase64BgImage(el){\n"
             "  try{\n"
             ...
             "  return false;\n"
             "}\n"
             "function scanBase64BgImages(root){\n"
             "  try{\n"
             "    var scope=root||document;\n"
             "    var all=scope.querySelectorAll('[style*=\"background-image\"],[style*=\"background:\"]');\n"
             "    for(var i=0;i<all.length;i++){ __stripBase64BgImage(all[i]); }\n"
             "  }catch(e){}\n"
             "}\n"
             "if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',function(){scanBase64BgImages();});}else{scanBase64BgImages();}\n"
             "setTimeout(scanBase64BgImages,500);\n"
             "setTimeout(scanBase64BgImages,1500);\n"
             "setTimeout(scanBase64BgImages,3000);\n"
             "setInterval(scanBase64BgImages,3000);\n"
             "try{\n"
             "  var __bgObs=new MutationObserver(function(mutations){\n"
             "    for(var m=0;m<mutations.length;m++){\n"
             "      var mut=mutations[m];\n"
             "      if(mut.type==='attributes'&&mut.attributeName==='style'){\n"
             "        __stripBase64BgImage(mut.target);\n"
             "        continue;\n"
             "      }\n"
             ...
             "        __stripBase64BgImage(node);\n"
             "        if(node.querySelectorAll){scanBase64BgImages(node);}\n"
             "      }\n"
             "    }\n"
             "  });\n"
             "  if(document.documentElement){\n"
             "    __bgObs.observe(document.documentElement,{\n"
